...
const App = () => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Race condition to prevent infinite loading on bad config
        await Promise.race([
          signInAnonymously(auth),
          new Promise((_, reject) => setTimeout(() => reject(new Error("Auth Timeout")), 2500))
        ]);
      } catch (error) {
        console.error("Auth failed or timed out", error);
        // Fallback for demo mode or invalid config
  ...
  useEffect(() => {
    ...
    const unsubUser = onSnapshot(userDocRef, (snap) => {
      if (snap.exists()) {
        ...
        // Only try to write if we think we can (skip if demo-user and likely no permissions)
        if (user.uid !== 'demo-user') {
             setDoc(userDocRef, data).catch(e => console.error("SetDoc failed", e));
        }
        setUserData(data);
      }
    }, (error) => {
        console.error("UserData snapshot error", error);
        setUserData({
    ...
    const unsubLb = onSnapshot(lbRef, (snap) => {
      ...
      setLeaderboard(docs.sort((a, b) => b.score - a.score).slice(0, 5));
        ...
        setLeaderboard([
            { name: "Demo Player 1", score: 5000, role: "Demo" },
            { name: "Demo Player 2", score: 3000, role: "Demo" }
        ]);
    });
