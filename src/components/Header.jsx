...
const Header = ({ userData }) => (
  <header className="sticky top-0 z-40 bg-black text-white px-6 py-4 flex items-center justify-between shadow-xl">
    <div className="flex items-center gap-3">
      ...
      <span className="text-[10px] font-bold tracking-[0.3em] text-zinc-500 uppercase hidden sm:block">Wiki 2.0</span>
    </div>
    {userData && (
      <div className="flex items-center gap-3 bg-zinc-900 px-3 py-1.5 rounded-full border border-zinc-800">
        <div className="text-right">
          <p className="text-[10px] font-bold leading-none text-zinc-400 uppercase tracking-tighter">{userData.level}</p>
          <p className="text-xs font-black">{userData.points} PTS</p>
        </div>
